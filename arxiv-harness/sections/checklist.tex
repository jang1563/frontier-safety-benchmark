...
\item {\bf Claims}
    ...
    \item[] Justification: The abstract and introduction clearly state the three contributions (contamination-aware design, structured mitigation auditing, release governance) and the empirical findings across six models. All claims are supported by results in \Cref{sec:results}.
...
\item {\bf Experiment statistical significance}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: We report 95\% bootstrap confidence intervals (10,000 resamples) for all means and deltas in \Cref{tab:main_results}. Mitigation effectiveness is tested using Wilcoxon signed-rank tests (paired by item) with significance markers in the main table. Effect sizes (Cohen's $d$, rank-biserial correlation) are reported. Cross-model differences are assessed using Friedman tests.

\item {\bf Experiments compute resources}
    \item[] Question: For each experiment, does the paper provide sufficient information on the computer resources (type of compute workers, memory, time of execution) needed to reproduce the experiments?
    \item[] Answer: \answerYes{}
    \item[] Justification: All inference is performed via cloud API calls (OpenAI, Anthropic, Google, DeepSeek, Together AI, Groq). No local GPU computation is required. The 288 API calls (6 models $\times$ 24 items $\times$ 2 conditions) complete in under 45 minutes total.
...
\item {\bf Declaration of LLM usage}
    ...
    \item[] Answer: \answerYes{}
    \item[] Justification: The core contribution is an evaluation framework for LLMs. The six evaluated models (GPT-4o, Claude Sonnet 4, Gemini 2.5 Pro, DeepSeek-V3, Llama-3.3-70B, Qwen3-32B) are fully described in \Cref{sec:experiments}. Additionally, an LLM-as-judge protocol using Claude Sonnet 4 as independent scorer is used for scoring validation (\Cref{sec:llm_judge,sec:validation}).
